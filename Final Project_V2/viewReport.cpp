...
void viewReport(const string& username, double bmr) {
    ...
    double progress = (adjustedBMR > 0) ? (totalCalories / adjustedBMR) : 0;
    if (progress > 1.5) progress = 1.5; // Cap bar display

    int barWidth = 40;
    int filled = static_cast<int>(barWidth * min(progress, 1.0));

    string color;
    if (totalCalories < adjustedBMR * 0.8)
        color = "\033[33m"; // Yellow (below goal)
    else if (totalCalories > adjustedBMR * 1.1)
